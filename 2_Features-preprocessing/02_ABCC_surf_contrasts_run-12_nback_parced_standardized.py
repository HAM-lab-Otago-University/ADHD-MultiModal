...
# %%
## set variable for each run task = WM
...
def prepare_contrasts(design_mat):
    ...
    contrasts = {
    'place': (
    0.5*contrasts['two-Back-Place']
    + 0.5*contrasts['zero-Back-Place'] ),
    'face': (
    0.166*contrasts['two-Back-NegFace']
    + 0.166*contrasts['zero-Back-NegFace'] 
    + 0.166*contrasts['two-Back-NeutFace']
    + 0.166*contrasts['zero-Back-NeutFace'] 
    + 0.166*contrasts['two-Back-PosFace']
    + 0.166*contrasts['zero-Back-PosFace'] ),
    'emotionface': (
    0.25*contrasts['two-Back-NegFace']
    + 0.25*contrasts['zero-Back-NegFace'] 
    + 0.25*contrasts['two-Back-PosFace']
    + 0.25*contrasts['zero-Back-PosFace'] ),
    'face-place': (
    0.166*contrasts['two-Back-NegFace']
    + 0.166*contrasts['zero-Back-NegFace'] 
    + 0.166*contrasts['two-Back-NeutFace']
    ...
    'PosFace-NeutFace': (
    0.5*contrasts['two-Back-PosFace']
    + 0.5*contrasts['zero-Back-PosFace'] 
    - 0.5*contrasts['two-Back-NeutFace']
    - 0.5*contrasts['zero-Back-NeutFace'] 
    ),
    'NegFace-NeutFace': (
    0.5*contrasts['two-Back-NegFace']
    + 0.5*contrasts['zero-Back-NegFace'] 
    - 0.5*contrasts['two-Back-NeutFace']
    - 0.5*contrasts['zero-Back-NeutFace'] 
    ),
    ...
    'twobk': (
    0.25*contrasts['two-Back-NegFace']
    + 0.25*contrasts['two-Back-NeutFace']
    + 0.25*contrasts['two-Back-PosFace']
    + 0.25*contrasts['two-Back-Place']
    ),
    'zerobk': (
    0.25*contrasts['zero-Back-NegFace'] 
    + 0.25*contrasts['zero-Back-NeutFace'] 
    + 0.25*contrasts['zero-Back-PosFace'] 
    + 0.25*contrasts['zero-Back-Place'] 
    ),
    'twobk-zerobk': (
    0.25*contrasts['two-Back-NegFace']
    - 0.25*contrasts['zero-Back-NegFace'] 
...
def surf_contrast(iter, sub_label, run, task_label, data_dir, output_dir):
    try:
        ...
        for run_ind in range(run_num):
            fmriprep_bold = (data_dir + 'derivatives/abcd-hcp-pipeline/sub-%s/ses-baselineYear1Arm1/func/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_bold_timeseries.dtseries.nii' %(sub_label, sub_label, run[run_ind]))
            eve_dir = (data_dir + 'task-start-offset/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_events.tsv' %(sub_label, run[run_ind]))
            conf_dir = (data_dir + 'derivatives/fmriprep/sub-%s/ses-baselineYear1Arm1/func/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_desc-confounds_timeseries.tsv' %(sub_label, sub_label, run[run_ind]))
...
plt.ioff()
output_dir = "/media/hcs-sci-psy-narun/ABCC/fmriresults01/derivatives/nilearn_glm/surf_nback_std_fixed_newcntr/"
if not os.path.exists(output_dir):
    os.mkdir(output_dir)
# model_and_args = zip(models, models_run_imgs, models_events, models_confounds)
with parallel_backend('loky', n_jobs=30):
