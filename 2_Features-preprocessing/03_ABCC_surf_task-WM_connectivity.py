...
# %%
subject_ID_df  = pd.read_csv('/media/hcs-sci-psy-narun/ABCC/fmriresults01/withcalib_list.tsv', sep = '\t')
...
def manage_plots(unparced_z, unparced_eff , sub_id, contrast_t, task_label, dir, output_dir):

     surface_zs = nl.plotting.view_surf(hcp.mesh.inflated,
     hcp.cortex_data(unparced_z), 
     bg_map=hcp.mesh.sulc, cmap='bwr')
     surface_zs.save_as_html(output_dir + '/sub-%s_task-%s_run-%s_contrast-%s_z_parcelated_plot.html' %(sub_id, task_label, dir, contrast_t))
     surface_eff = nl.plotting.view_surf(hcp.mesh.inflated,
     hcp.cortex_data(unparced_eff), 
     bg_map=hcp.mesh.sulc, cmap='bwr')
     surface_eff.save_as_html(output_dir + '/sub-%s_task-%s_run-%s_contrast-%s_effect_parcelated_plot.html' %(sub_id, task_label, dir, contrast_t))
     plt.close('all')
     gc.collect()
...
def surf_conn(iter, sub_label, dir, task_label, data_dir, output_dir):
    try:
        ...
        for dir_ind in range(dir_num):
            # specify directories
            fmriprep_bold = (data_dir + 'derivatives/abcd-hcp-pipeline/sub-%s/ses-baselineYear1Arm1/func/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_bold_timeseries.dtseries.nii' %(sub_label, sub_label, run[dir_ind]))
            eve_dir = (data_dir + 'task-start-offset/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_events.tsv' %(sub_label, run[dir_ind]))
            # eve_dir = ('/media/hcs-sci-psy-narun/ABCC_F/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_events.tsv' %(sub_label, run[run_ind]))
            conf_dir = (data_dir + 'derivatives/fmriprep/sub-%s/ses-baselineYear1Arm1/func/sub-%s_ses-baselineYear1Arm1_task-nback_run-%s_desc-confounds_timeseries.tsv' %(sub_label, sub_label, run[dir_ind]))
            ...
            ind_events.to_csv(output_dir + 
                            'sub-%s/sub-%s_task-%s_run-%s_events.tsv' %(sub_label, sub_label, task_label, dir[dir_ind]), sep='\t')
        ...
        if complete_run:
            ...
            full_cen_res = np.concatenate((cen_res_list[0],cen_res_list[1]), axis=0)
            ...
            full_uncen_res = np.concatenate((uncen_res_list[0],uncen_res_list[1]), axis=0)
        ...
        pd.DataFrame({'round': [dir[0], dir[1] , dirM ], 'del vols': censored_vols, 'N of vols needed': half_vols, 'high motion vols': outlier_vols}).to_csv(path + 
...
#plt.ioff()
output_dir = data_dir + "derivatives/nilearn_glm/tconnectivity_WM/fullscrubbed-despiked/"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

Parallel(n_jobs=20)(delayed(surf_conn)(iter, subject_label, run, task_label, data_dir, output_dir) for iter, subject_label in enumerate(subject_ID))
