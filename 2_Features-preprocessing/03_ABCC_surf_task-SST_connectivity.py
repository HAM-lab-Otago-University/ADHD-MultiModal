...
output_dir = data_dir + "derivatives/nilearn_glm/tconnectivity_SST/fullscrubbed-despiked/"
...
def convert_to_cifti (stat_map, axes, t_r):
    ...
    scalar_axis_map = nb.cifti2.SeriesAxis(start=0, step=t_r, size=map.shape[0])  
...
def flag_outliers(confounds):
    fd_threshold = 0.5  #from xcp-d #.3 in hagler paper for rest, .9 in hagler paper for task
...
def surf_conn(iter, sub_label, dir, task_label, data_dir, output_dir):
    try:
        ...
        for dir_ind in range(dir_num):
            ...
            eve_dir = (data_dir + 'sst_events/sub-%s_ses-baselineYear1Arm1_task-%s_run-%s_events.tsv' %(sub_label, task_label, dir[dir_ind]))
