...
output_dir = data_dir + "derivatives/nilearn_glm/tconnectivity_MID/fullscrubbed-despiked_onset/"
...
subject_ID_df  = pd.read_csv(home_dir + 'ABCC_MID_subject_list3.tsv', sep = '\t')
...
def surf_conn(iter, sub_label, dir, task_label, data_dir, output_dir):
    try:
        ...
        for dir_ind in range(dir_num):
            # specify directories
            fmriprep_bold = (data_dir + 'derivatives/abcd-hcp-pipeline/sub-%s/ses-baselineYear1Arm1/func/sub-%s_ses-baselineYear1Arm1_task-%s_run-%s_bold_timeseries.dtseries.nii' %(sub_label, sub_label, task_label, dir[dir_ind]))
            eve_dir = (data_dir + 'mid_events_2024_onset/sub-%s_ses-baselineYear1Arm1_task-%s_run-%s_events.tsv' %(sub_label, task_label, dir[dir_ind]))
            conf_dir = (data_dir + 'derivatives/fmriprep/sub-%s/ses-baselineYear1Arm1/func/sub-%s_ses-baselineYear1Arm1_task-%s_run-%s_desc-confounds_timeseries.tsv' %(sub_label, sub_label, task_label, dir[dir_ind]))
...
Parallel(n_jobs=25)(delayed(surf_conn)(iter, subject_label, run, task_label, data_dir, output_dir) for iter, subject_label in enumerate(subject_ID))
