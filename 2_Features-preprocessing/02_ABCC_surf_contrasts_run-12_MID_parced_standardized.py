...
# %%
subject_ID_df  = pd.read_csv('/media/hcs-sci-psy-narun/ABCC/fmriresults01/ABCC_MID_subject_list3.tsv', sep = '\t')
...
def prepare_contrasts(design_mat):
    ...
    final_contrasts = {
    'Reward-Neutral': (
    0.5*contrasts['LgReward']
    + 0.5*contrasts['SmallReward']
    - contrasts['Neutral'] 
    ),
    'Loss-Neutral': (
    0.5*contrasts['LgPun']
    + 0.5*contrasts['SmallPun']
    - contrasts['Neutral'] 
    ),
    'LgReward-Neutral': (
    contrasts['LgReward']
    ...
    0.5*contrasts['LgReward Hit']
    + 0.5*contrasts['SmallReward Hit'] 
    - 0.5*contrasts['LgReward Miss'] 
    - 0.5*contrasts['SmallReward Miss'] 
    ),
    'LossHit-LossMiss': (
    0.5*contrasts['LgPun Hit']
    + 0.5*contrasts['SmallPun Hit'] 
    - 0.5*contrasts['LgPun Miss'] 
    - 0.5*contrasts['SmallPun Miss'] 
    )
    }
...
def surf_contrast(iter, sub_label, run, task_label, data_dir, output_dir):
    try:
        ...
        for run_ind in range(run_num):
            ...
            eve_dir = (data_dir + 'mid_events_2024_onset/sub-%s_ses-baselineYear1Arm1_task-%s_run-%s_events.tsv' %(sub_label, task_label, run[run_ind]))
...
plt.ioff()
output_dir = "/media/hcs-sci-psy-narun/ABCC/fmriresults01/derivatives/nilearn_glm/surf_MID_std_fixed_newcntr/"
...
for id in subject_ID:
    ...
    if diris == False:
        ...
        completed = os.path.isfile(output_dir +'sub-%s/sub-%s_task-nback_run-mean_contrast-twobk-zerobk_space-fsLR_den-91k_stat-effect_parcellated.dtseries.nii' %(id , id))
        if completed == False:
