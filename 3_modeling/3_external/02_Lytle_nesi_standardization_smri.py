...
def run_std(te_index, path_out):
    try:
        ...
        y_te_std = pd.DataFrame(y_scaler.fit_transform(y_te_r),  index=y_te.index)
        ...
        for key, feature in features.items():
            try:
                print(key, te_index, feature.index)
                ...
                print(key, x_te.shape, x_te.index)
...
cog_cols = ['WASI_VIQ', 'WASI_PIQ', 'WASI_FSIQ']
for y_name in cog_cols:
     y = targs[y_name]
     target_name =  y_name.split('_')[1] + '_'
