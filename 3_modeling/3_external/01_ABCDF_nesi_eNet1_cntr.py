...
def run_enet1(path_out, target_name):
    try:
        ...
        search.fit(x_comp.values, y_comp.values)
        ...
        #     joblib.dump(best_model, path_out + 'enet-l1_best_model_'+ target_name +str(key)+'.pkl')
            ...
            tr_y_eN1 = pd.DataFrame(tr_y_p, index=y_comp.index)
            ...
            # fill output dict
            enet1_dict[key]['data'].update({'yptrain': tr_y_eN1, 'yttrain': y_comp})  
...
for targ in targ_list:
    ...
    # save dict
    joblib.dump(enet1_dict, enet1_dir + '/' + targ + feature_set[0:4] + '_enet1_output_fstd.joblib', compress=0) 
