...
def run_pls(path_out, target_name, pls_dict):
    try:
        ...
        grid_search.fit(x_tr.values, y_tr.values)
        ...
        tr_x_pls = pd.DataFrame(best_model.transform(x_tr.values), index=x_tr.index)
        # tr_x_pls.to_csv(path_out +'/' + target_name + str(key) + '_pls_train'+'.csv')
        # predict train y
        tr_y_p = best_model.predict(x_tr.values)
        tr_y_pls = pd.DataFrame(tr_y_p, index=y_tr.index)#.to_csv(path_out + '/predicted_y_'+ target_name + str(key) + '_pls_train'+'.csv')
        ...
        # Transform the train using the PLS model
        te_x_pls = pd.DataFrame(best_model.transform(x_te.values), index=x_te.index)
        # te_x_pls.to_csv(path_out + '/' + target_name + str(key) + '_pls_test'+'.csv')

        # predict test y
        te_y_p = best_model.predict(x_te.values)
        te_y_pls = pd.DataFrame(te_y_p, index=y_te.index)#.to_csv(path_out + '/predicted_y_'+ target_name + str(key) + '_pls_test'+'.csv')
        ...
        pls_dict[key]['data'].update({'Xtrain': tr_x_pls, 'Xtest1': te_x_pls, 'yptrain': tr_y_pls, 'yptest': te_y_pls, 'yttrain': y_tr, 'yttest': y_te}) 
        pls_dict[key]['model'].update({'perf': performance, 'cv_results': grid_search.cv_results_, 'best_model': best_model, 'coef': best_model.coef_}) 
        gc.collect()
...
for targ in targ_list:
    ...
    for key, feature in std_features.items():
        ...
        y_tr = target['train']['std'].dropna() #raw
    ...
    # save dict
    joblib.dump(pls_dict, pls_dir + '/' + targ + feature_set[0:4] + '_pls_output_std.joblib', compress=0) 
