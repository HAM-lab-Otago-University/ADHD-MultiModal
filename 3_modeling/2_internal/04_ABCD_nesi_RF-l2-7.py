...
moda_stack_dict = dict(
                        ...
                        nD_WMAll = nbk_list + ['conn_wm'],
                        nD_MidAll = mid_list+ ['conn_mid'],
                        nD_SstAll = sst_list+ ['conn_sst'],
...
for targ in targ_list:
    ...
    mnct = joblib.load(enet1_dir + targ + 'mnct_enet1_output_std.joblib')
    
    # conn2 = joblib.load(enet1_dir + targ + 'con2_enet1_output.joblib')
    # tang = joblib.load(enet1_dir + targ + 'tang_enet1_output.joblib')
    All_enet1 = {**smri, **cntr, **conn, **gtfc, **abcd_rsmri, **abcd_cntr, **mnct} # , **tang, **conn2
    ...
    # save dict
    joblib.dump(rf2_dict, randfor2_dir + targ + 'rf2-7_output_std.joblib', compress=0)
    joblib.dump(rf2_model, randfor2_dir + targ + 'rf2-7_model_std.joblib', compress=0) 
    # Save rf2_dict using pickle
    with open(randfor2_dir + targ + 'rf2-7_output_std.pkl', 'wb') as f:
        pickle.dump(rf2_dict, f)
    with open(randfor2_dir + targ + 'rf2-7_model_std.pkl', 'wb') as f:
