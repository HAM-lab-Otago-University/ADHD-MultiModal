...
moda_stack_dict = dict(
                        ...
                        stacked_all = ['DTI', 'VolBrain', 'T1_white', 'T1_gray', 'T2_white', 'T2_gray', 'Sulcal_Depth', 'T1_norm', 'T2_norm', 'T1_summ','T2_summ','conn_rest', 
                                'conn_wm', 'conn_sst', 'conn_mid', 'subnet_rest', 'gfc', 'tfc'] 
                                 + contrasts_mid + contrasts_sst + contrasts_wm  + abcd_contrasts + abcd_rsmri, 
                        GD_WMCntr = contrasts_wm + nbk_list,
                        GD_MidCntr = contrasts_mid + mid_list,
                        GD_SstCntr = contrasts_sst + sst_list,
                        GD_TaskCntr = contrasts_wm + nbk_list + contrasts_mid + mid_list + contrasts_sst + sst_list,
                        ...
                        GD_TaskAll = contrasts_wm + nbk_list + contrasts_mid + mid_list + contrasts_sst + sst_list + ['conn_wm', 'conn_sst', 'conn_mid'],
                        D_WMCntr = nbk_list,
                        D_MidCntr = mid_list,
                        D_SstCntr = sst_list,
...
for targ in targ_list:
    ...
    # save dict
    joblib.dump(rf2_dict, randfor2_dir + targ + 'rf2-6_output_std.joblib', compress=0)
    joblib.dump(rf2_model, randfor2_dir + targ + 'rf2-6_model_std.joblib', compress=0) 
    # Save rf2_dict using pickle
    with open(randfor2_dir + targ + 'rf2-6_output_std.pkl', 'wb') as f:
        pickle.dump(rf2_dict, f)
    with open(randfor2_dir + targ + 'rf2-6_model_std.pkl', 'wb') as f:
