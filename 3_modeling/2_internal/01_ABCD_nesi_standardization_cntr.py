...
# %%
def run_std(y, tr_index, te_index, path_out, target_name):
    try:
        ...
        for key, feature in features.items():
            try:
                ...
                #add to dict
                std_features[key]['train'] = x_tr_std
                std_features[key]['test'] = x_te_std

                gc.collect()
            except Exception as e:
                logging.error(traceback.format_exc())
        # save dict
        joblib.dump(std_features, path_out + '/std_features.joblib', compress=1) 
...
for y_name in cog_cols:
    ...
    run_std(y, train_index, test_index, main_fold, target_name)
